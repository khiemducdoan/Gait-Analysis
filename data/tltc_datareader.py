...
class TLTC_Reader():
    ...
    def __init__(self, joints_path_list, smpls_path, params):
        ...
        print(f"There are {len(self.pose_dict)} sequences in the T-LTC dataset.")
        print(f"There are {len(set(self.participant_ID))} different patients in the T-LTC dataset: {set(self.participant_ID)}")
    ...
    def read_keypoints(self):
        ...
        print('[INFO - TLTC_Reader] Reading body keypoints from npz')
        ...
        for joints_path in self.joints_path_list:
            ...
            for seq_name in tqdm([s for s in seqs.keys() if not s.endswith('_frame_ids')]):
                ...
                if joints is None:
                    print(f"[WARN - TLTC_Reader] {seq_name} is None.")
