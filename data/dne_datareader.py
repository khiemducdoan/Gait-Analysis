...
class DNE_Reader():
    ...
    def __init__(self, joints_path_list, labels_path, params):
        ...
        print(f"There are {len(self.pose_dict)} sequences in the DNE dataset.")
        print(f"There are {len(set(self.participant_ID))} different patients in the DNE dataset: {set(self.participant_ID)}")
        unique, counts = np.unique(list(self.labels_dict.values()), return_counts=True)
        print(f"Distribution of labels in DNE dataset:")
        print(np.asarray((unique, counts)).T)

    def read_label(self, seq_name):
        subject_id, walkid = seq_name.split("__")
        walkid = walkid.split("_down")[0]
        label = self.label_df[subject_id][walkid]['other']
        return label
    ...
    def read_keypoints_and_labels(self):
        ...
        print('[INFO - DNE_Reader] Reading body keypoints from npz')
        ...
        for joints_path in self.joints_path_list:
            ...
            for seq_name in tqdm([s for s in seqs.keys() if not s.endswith('_frame_ids')]):
                ...
                if joints is None:
                    print(f"[WARN - DNE_Reader] {seq_name} is None.")
